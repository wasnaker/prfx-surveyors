...
<div
    ...
    <?php if (count($surveyors_years) > 1) { ?>
    <div class="simple-bootstrap-select">
        <select
            data-none-selected-text="<?= date('Y'); ?>"
            data-width="fit" class="selectpicker tw-w-full tw-min-w-[79px]" multiple name="surveyors_total_years"
            onchange="init_surveyors_total();">
...
foreach ($totals as $key => $data) {
    $class = surveyor_status_color_class($data['status']);
    $name  = surveyor_status_by_id($data['status']); ?>
    <div
